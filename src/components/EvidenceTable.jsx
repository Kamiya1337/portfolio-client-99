...
export default function EvidenceTable() {
  ...
  return (
    <div className="min-h-screen">
      {previewData.isOpen && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-charcoal/55 p-3 backdrop-blur-md sm:p-6" role="dialog" aria-modal="true" aria-label="Xem trước minh chứng"><div className="glass-panel flex h-[90vh] w-full max-w-6xl flex-col overflow-hidden rounded-glass-lg"><div className="flex items-center justify-between border-b border-border-light px-5 py-4"><div><p className="editorial-label">Evidence Preview</p><h3 className="mt-1 font-display text-2xl font-bold">{previewData.type === 'pdf' ? 'Báo cáo PDF' : 'Ảnh minh chứng'}</h3></div><div className="flex gap-2"><a href={previewData.url} target="_blank" rel="noreferrer" className="button-secondary">Mở thẻ mới</a><button type="button" aria-label="Đóng xem trước" onClick={() => setPreviewData({ isOpen: false, url: '', type: '' })} className="icon-button-dark"><X size={18} /></button></div></div><div className="flex min-h-0 flex-1 items-center justify-center overflow-auto bg-white/40 p-4">{previewData.type === 'pdf' ? <iframe src={previewData.url} title="PDF Preview" className="h-full w-full rounded-xl border-0 bg-white" /> : <img src={previewData.url} alt="Minh chứng" className="max-h-full max-w-full rounded-xl object-contain" />}</div></div></div>}
      ...
      <section className="px-5 pb-28 sm:px-8 xl:px-12">
        <div className="glass-panel mx-auto max-w-[1220px] overflow-hidden rounded-glass-lg">
          <div className="divide-y divide-border-light md:hidden">{projects.map((project) => <article key={project.id} className="p-6"><div className="flex items-start justify-between gap-4"><div><p className="editorial-label">Assignment {String(project.id).padStart(2, '0')}</p><h2 className="mt-3 font-display text-2xl font-bold">{project.title}</h2></div><StatusBadge project={project} /></div><div className="mt-6 grid grid-cols-2 gap-2"><EvidenceLink value={project.report} label="Report" type="pdf" /><EvidenceLink value={project.evidenceImg} label="Image" type="img" /></div></article>)}</div>
          <div className="hidden overflow-x-auto md:block"><table className="w-full min-w-[760px] border-collapse text-left"><thead><tr className="border-b border-border-light bg-white/30 text-[11px] font-bold uppercase tracking-[0.14em] text-muted-dark"><th className="px-7 py-6">Assignment</th><th className="px-7 py-6">Report File</th><th className="px-7 py-6">Evidence Image</th><th className="px-7 py-6">Status</th></tr></thead><tbody>{projects.map((project) => <tr key={project.id} className="border-b border-border-light transition-colors hover:bg-white/55"><td className="px-7 py-8"><p className="font-display text-xl font-bold">{project.title}</p><p className="mt-2 text-xs uppercase tracking-wider text-muted-dark">{project.chapter}</p></td><td className="px-7 py-8"><EvidenceLink value={project.report} label="Xem báo cáo" type="pdf" /></td><td className="px-7 py-8"><EvidenceLink value={project.evidenceImg} label="Xem ảnh" type="img" /></td><td className="px-7 py-8"><StatusBadge project={project} /></td></tr>)}</tbody></table></div>
